...
- E-category checks require @explore delegation and are only run when other categories already indicate SUSPECTED or
  CONFIRMED.
...
Analyze project awareness. All E-category checks require @explore delegation. Skip E-category entirely if no other
...
- The diff, diff stat, and log arrive via one `@explore` delegation (verbatim output) — you never run bash/git
  yourself; detection categories run over the diff text in your own context
- Beyond-diff context (conventions, duplicates, callers, surrounding code) comes from `@explore` — one aggregated
  query; your only `read` grant is the reference tier (`reference/*.md`)
