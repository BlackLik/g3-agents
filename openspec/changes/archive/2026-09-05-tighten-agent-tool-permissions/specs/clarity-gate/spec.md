...
After the analysis phase (and any `@explore` context pass) and before the first `@player` delegation, `flow` SHALL
assess whether it is materially blocked. If so, `flow` SHALL ask the user at most ONE round of questions via the
`question` tool before delegating implementation. Materially blocked means: ambiguous success criteria, a destructive
or irreversible choice, or missing access. If not materially blocked, `flow` SHALL proceed on stated assumptions
without asking.
...
- **THEN** flow SHALL incorporate the answers into the delegation prompts
